...
void solve() {
    ...
    for(ll i=0;i<n;i++){
        ll pos=i+1;
        if(hor[i]=='>'){
            for(ll j=1;j<m;j++)graph[{pos,j}].pb({pos,j+1});
            }
        else {
            for(ll j=m;j>1;j--)graph[{pos,j}].pb({pos,j-1});
        }
    }
    for(ll j=0;j<m;j++){
        ll pos=j+1;
        if(ver[j]=='v'){
            for(ll i=1;i<n;i++)graph[{i,pos}].pb({i+1,pos});
            }
        else {
            for(ll i=n;i>1;i--)graph[{i,pos}].pb({i-1,pos});
        }
    }
    for(auto x:graph){
        test=0;
        again();
        dfs(x.F);
        if(test!=n*m){cout<<"NO"<<endl;return;}
    }
    cout<<"YES"<<endl;
}
int main()
